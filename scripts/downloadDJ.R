...
#################################
if (length(args) != 3) {
  stop("Error: Three arguments must be provided:\n\t1. Initial page number\n\t2. Final page number\n\t3. Output folder to save pdf documents", call.=FALSE)
  ...
  cat("Passed final page is smaller than initial page. The order was changed to attend the program requisites")
  aux = args[1]
  args[1] = args[2]
  args[2] = aux
}
...
downloadDiarie = function(edition, complementURL){
  ...
  tryCatch({
    download.file(paste0(baseURL,edition,complementURL),paste0(outputFolder,edition,complementURL))
  },
  error = function(cond){
    return(NA)
  }) 
...
editions = pbmclapply(initialPage:finalPage, getEditions, mc.cores = ncores, ignore.interactive = T)
...
if(length(newerEditions) > 0){
  pbmclapply(newerEditions, downloadDiarie, complementURL = " C1 Tribunal de Justiça.pdf", mc.cores = ncores, ignore.interactive = T)
  pbmclapply(newerEditions, downloadDiarie, complementURL = " C2 Comarcas - Entrância Especial.pdf", mc.cores = ncores, ignore.interactive = T)
  pbmclapply(newerEditions, downloadDiarie, complementURL = " C7 Comarcas - 1ª 2ª e 3ª Entrância.pdf", mc.cores = ncores, ignore.interactive = T)
  pbmclapply(newerEditions, downloadDiarie, complementURL = " C6 Foro Extrajudicial.pdf", mc.cores = ncores, ignore.interactive = T)
}

if(length(olderEditions) > 0){
  pbmclapply(olderEditions, downloadDiarie, complementURL = ".pdf", mc.cores = ncores, ignore.interactive = T)
}
